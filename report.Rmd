...
pFtest(plm_ap, ols)  # ------- fixed effect model better 
######### # alternative way
library(lfe)
...
phtest(plm_ap, random) # ------ choose fixed model 
...
pFtest(plm_ap, time)
plmtest(plm_ap, c("time"), type=("bp")) # ----no need to use time-fixed effects
##########  Testing for cross-sectional dependence
pcdtest(plm_ap, test = c("lm")) # ------ ??? Cross-sectional dependence
pcdtest(plm_ap, test = c("cd")) # ------No cross-sectional dependence
######### # Testing for serial correlation
pbgtest(plm_ap) # ------ serial correlation exist
...
coeftest(plm_ap)
coeftest(plm_ap, vcovHC)
coeftest(plm_ap, vcovHC(fixed, method = "arellano"))
coeftest(plm_ap, vcovHC(fixed, type = "HC3")) 
t(sapply(c("HC0", "HC1", "HC2", "HC3", "HC4"), function(x) sqrt(diag(vcovHC(plm_ap, type = x))))) # ------ coefficient not highly correlated

#########  multicollinearity ############
design_matrix <- as.data.frame(model.matrix(plm_ap))
...
#########  Homoscedasticity ############
par(bg='#eeeeee')
fitted.values<-as.numeric(data$afatal_prop - residuals(plm_ap))
plot(fitted.values,residuals(plm_ap),bty='n',xlab='Fitted values', ylab='Residuals',main='Residuals vs. Fitted')
lines(smooth.spline(fitted.values,residuals(plm_ap), spar=0.9), col=2)
abline(h=0,col='red',lty='dashed')
# studentized residuals
X <- model.matrix(plm_ap)
P <- X %*% solve(t(X) %*% X) %*% t(X)
sigma.sq <- (1 /plm_ap$df.residual)*sum(residuals(plm_ap)^2)
student.res <- residuals(plm_ap)/(sigma.sq * (1-diag(P)))

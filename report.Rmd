...
plm_final <- plm(afatal_prop~spirits+unemp+drinkage+jail+service+breath,data=data,index=c("state", "year"),model="within")
summary(plm_final)
...
######### # alternative way
library(lfe)
lfe_ap <- lfe::felm(afatal_prop~spirits+unemp+beertax+drinkage+dry+breath+jail+service|state,data=data)
summary(lfe_ap)
######### # Hausman test: fixed vs random model
random <- plm(afatal_prop~spirits+unemp+beertax+drinkage+dry+breath+jail+service,data=data,index=c("state", "year"),model="random")
phtest(plm_final, random) # ------ choose fixed model 
######### # Testing for time-fixed effects
time <- plm(afatal_prop~spirits+unemp+beertax+drinkage+dry+breath+jail+service+year,data=data,index=c("state", "year"),model="within")
pFtest(plm_final, time)
...
pcdtest(plm_final, test = c("cd")) # ------No cross-sectional dependence
######### # Testing for serial correlation
pbgtest(plm_final) # ------ serial correlation exist
...
library(lmtest)
bptest(afatal_prop~spirits+unemp+beertax+drinkage+dry+breath+jail+service+state, data =data, studentize=F) # ----------- Presence of heteroskedasticity 
##########  Controlling for heteroskedasticity: Robust covariance matrix estimation (Sandwich estimator)
coeftest(plm_final)
coeftest(plm_final, vcovHC)
coeftest(plm_final, vcovHC(fixed, method = "arellano"))
coeftest(plm_final, vcovHC(fixed, type = "HC3")) 
t(sapply(c("HC0", "HC1", "HC2", "HC3", "HC4"), function(x) sqrt(diag(vcovHC(plm_final, type = x))))) # ------ coefficient not highly correlated
...
lm_fit1 <- lm(afatal_prop~spirits+unemp+beertax+drinkage19+drinkage20+drinkage21+dry+breathyes+jailyes+serviceyes, data=design_matrix)
# VIF score
car::vif(lm_fit1) #-------- small VIF scores
#########  Homoscedasticity ############
par(bg='#eeeeee')
fitted.values<-as.numeric(data$afatal_prop - residuals(plm_final))
plot(fitted.values,residuals(plm_final),bty='n',xlab='Fitted values', ylab='Residuals',main='Residuals vs. Fitted')
lines(smooth.spline(fitted.values,residuals(plm_final), spar=0.9), col=2)
...
plot(fitted.values,student.res,bty='n',xlab='Fitted values', ylab='Residuals',main='Studentized Model residuals vs. Fitted')
lines(smooth.spline(fitted.values,student.res, spar=0.9), col=2)
abline(h=0,col='red',lty='dashed') # -------- suspect heteroscedasticity
#########  Influential Observations ############
require(faraway)
halfnorm(diag(P),labs=1:335, ylab='Leverages',nlab=1) # 2p/n=0.06
data[287,] # --- ut 1982
data %>% filter(state=="ut") # ut 1982 beertax low
...
However, the model diagnosis result showed that our model had cross-sectional dependence, which is a violation of the model assumption that states should be independent of each other. Although each state had their own legislative system, the federal law can still largely affect state laws. 

However, a fixed effect model requires exogeneity assumptions in order to make causal inference, including: (a) no unobserved time-varying confounders; (b) past outcomes do not directly affect current outcome; (c) past treatments do not directly affet current outcome; (d) past outcome do not directly affect current treatment (reverse causation). 
